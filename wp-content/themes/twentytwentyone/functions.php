 ...
 */
function wpdocs_create_job_taxonomies() {
  ...
  register_taxonomy( 'job-category', array( 'job' ), $args );
	...
	$labels = array(
    'name' => _x( 'Job tag', 'taxonomy general name' ),
    'singular_name' => _x( 'Job tag', 'taxonomy singular name' ),
    'search_items' =>  __( 'Search job tag' ),
    'all_items' => __( 'All job tag' ),
    'edit_item' => __( 'Edit Job tag' ), 
    'update_item' => __( 'Update Job tag' ),
    'add_new_item' => __( 'Add New Job tag' ),
    'new_item_name' => __( 'New Job tag Name' ),
    'menu_name' => __( 'Job tag' ),
  ); 	
...
add_action( 'init', 'wpdocs_create_job_taxonomies', 0 );
...
// ==========create shortcode================================================
function custom_button_FB_shortcode() {
	return '<a class="btn btn-primary rounded" href="https://www.facebook.com">Go to Facebook</a>';
}
add_shortcode( 'Button_FB', 'custom_button_FB_shortcode' );
...
function custom_image_shortcode($atts) {
	...
	$img_src = wp_get_attachment_image_src( $atts['src'], 'full' );
	
	return '<img src="' . $img_src[0] . '" alt="' . $atts['alt'] . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '">';
}
...
function custom_video_shortcode($atts) {
	...
	$video_src = wp_get_attachment_url( $atts['src'] );
	
	return '<video src="' . $video_src . '" poster="' . $atts['poster'] . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" ' . $atts['controls'] . '></video>';
}
    ...
    if($post_new->have_posts()):
        echo '<ul>';
            while($post_new->have_posts()):$post_new->the_post();
                echo '<li class="text-success">'.get_the_title().'</>';
            endwhile;
        echo '</ul>';
